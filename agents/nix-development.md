...
-   **Build Reproducibility**: Prioritize `nix build` for reproducible artifacts over `cabal build` outside a controlled Nix environment.
...
During the initial setup, significant challenges were encountered in establishing a working Haskell development environment using Nix flakes. Build errors (e.g., `cabal2nix` not finding .cabal files, GHC installation failures due to `FP_PROG_LD_BUILD_ID` errors and permission denied issues) indicated underlying environmental complexities or conflicts between Nix and system-level dependencies. These issues were ultimately resolved by adopting a pure Nix+Cabal workflow.
